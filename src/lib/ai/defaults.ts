...
export function buildSystemPrompt(args: {
  ...
  if (mode === 'auto_reply') {
    ...
    parts.push(
      `Service routing: when the lead makes clear WHICH service they're interested in, append the matching marker at the very end of your message so the specialist takes over from the next turn: [[SERVICE:VOICE_AI]] for the AI voice receptionist, [[SERVICE:WEBSITE_FUNNEL]] for websites/funnels, [[SERVICE:CHATBOT_LEADS]] for website chatbots/lead capture, [[SERVICE:COLD_EMAIL]] for cold email outreach, [[SERVICE:AI_CONTENT]] for AI clone/video content. If the conversation moves to a general question about the company or a DIFFERENT topic than the current specialist, append [[SERVICE:GLOBAL]] to hand back to the generalist. Only emit a marker when the lead's choice is clear — never guess, ask instead. Never mention these markers; they are stripped before sending.`,
    )
